...
function WebpackServiceWorker(params) {
  ...
  function cacheAssets(section) {
    return caches.open(CACHE_NAME).then(function (cache) {
      return cache.addAll(assets[section].map(function (url) {
        return url + '?v=' + params.hash;
      })).then(function () {
  ...
  self.addEventListener('fetch', function (event) {
    ...
    event.respondWith(fetch(event.request.clone())['catch'](function (err) {
    ...
    var resource = caches.match(request, {
      cacheName: CACHE_NAME
