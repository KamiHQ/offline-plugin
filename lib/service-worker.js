  ...
  _createClass(ServiceWorker, [{
    ...
    value: function getDataTemplate(data, plugin, minify) {
      ...
      var cache = function cache(key) {
        return (data[key] || []).map(plugin.relativePaths ? function (p) {
          return _path2['default'].join(_this.basePath, p);
        } : function (a) {
          return a;
        }).map(_this.publicPath ? function (p) {
          return _path2['default'].join(_this.publicPath, p);
        } : function (a) {
          return a;
        });
      };
    ...
    value: function getConfig(plugin) {
      return {
        output: _path2['default'].join(this.publicPath || plugin.scope, this.output),
        scope: plugin.scope
      };
    }
  }]);
